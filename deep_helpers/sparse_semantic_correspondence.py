...
class sparse_semantic_correspondence():
    def __init__(self, model, gpu_ids, tau, border_size, isolation_th, save_dir, k_per_level, k_final):
    ...
    def remove_isolated_correspondence(self, correspondence, image_size, isolation_threshold):
        assert(len(correspondence[0])==len(correspondence[1]))
        filtered_correspondence = correspondence
        points = correspondence[0]
        density_map = self.generate_density_map(points, image_size, kernel_width=23, stdv=10)
        for i in range(len(points)-1,-1,-1):
            if density_map[0, 0, points[i][0], points[i][1]] <= isolation_threshold:
                print("removing: ",i, ", value: ", density_map[0, 0, points[i][0], points[i][1]])
                filtered_correspondence[0].pop(i)
                filtered_correspondence[1].pop(i)
                filtered_correspondence[2].pop(i)

        assert(len(filtered_correspondence[0])==len(filtered_correspondence[1]))
        return filtered_correspondence
    ...
    def generate_density_map(self, points, image_size, kernel_width=5, stdv=1):
        delta_map = self.Tensor(1,1,image_size[2],image_size[3]).fill_(0)
        for k in range(len(points)):
            delta_map[0,0,points[k][0],points[k][1]] = 1

        kernel = self.gaussian(kernel_width, stdv=stdv)
        pad_size = math.floor(kernel_width/2)
        distance_map = functional.conv2d(Variable(delta_map), Variable(kernel.contiguous()), padding=pad_size).data

        return distance_map

    def gaussian(self, kernel_width, stdv = 1):
        w = self.identity_map([1,1,kernel_width,kernel_width]) - math.floor(kernel_width/2)
